...
def load_prompt(data_name, prompt_type, num_shots):
    ...
    if data_name in ["gsm_hard", "svamp", "tabmwp", "asdiv", "mawps"]:
        data_name = "gsm8k"
    if data_name in ["math_oai", "hungarian_exam", "math-oai", "aime24", "amc23", "aimo2", "aime25"]:
        data_name = "math"
...
PROMPT_TEMPLATES = {
    ...
    "deepseek-r1-no-think": (
     ...
     "deepseek-r1-choice": (
        ...
        "'Therefore, the final answer is: $\\boxed{{LETTER}}$' (without quotes) where LETTER is one of ABCD."
        "<｜User｜>{input}<｜Assistant｜><think>\n",
        "{output}",
        "\n\n",
    ),
